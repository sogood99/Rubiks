    ...
    def actionSelf(self, dir:direction, isRev:bool, num:int)->None:
        ...
        if (dir == direction.horz):
            ...
            if (not isRev):
                ...
                temp1:np.ndarray = deepcopy(self.config[5][num, :][::-1])
                self.config[5][num, :] = deepcopy(self.config[0][num, :])

                temp2 = deepcopy(self.config[2][reflectedNum, :][::-1])
                self.config[2][reflectedNum, :] = temp1

                temp1 = deepcopy(self.config[4][num, :])
                self.config[4][num, :] = temp2

                self.config[0][num, :] = temp1
                ...
                # faces = [0,4,2,5]
                temp1:np.ndarray = deepcopy(self.config[4][num, :][::-1])
                self.config[4][num, :] = deepcopy(self.config[0][num, :])

                temp2 = deepcopy(self.config[2][reflectedNum, :][::-1])
                self.config[2][reflectedNum, :] = temp1

                temp1 = deepcopy(self.config[5][num, :])
                self.config[5][num, :] = temp2
            ...
            if (not isRev):
                # uses [0,1,2,3]
                temp1:np.ndarray = deepcopy(self.config[1][:,num])
                self.config[1][:,num] = deepcopy(self.config[0][:,num])

                temp2 = deepcopy(self.config[2][:,num])
                self.config[2][:,num] = temp1

                temp1 = deepcopy(self.config[3][:,num])
                self.config[3][:,num] = temp2
                ...
                # uses [0,3,2,1]
                temp1:np.ndarray = deepcopy(self.config[3][:,num])
                self.config[3][:,num] = deepcopy(self.config[0][:,num])

                temp2 = deepcopy(self.config[2][:,num])
                self.config[2][:,num] = temp1

                temp1 = deepcopy(self.config[1][:,num])
                self.config[1][:,num] = temp2
            ...
            if (not isRev):
                ...
                temp1:np.ndarray = deepcopy(self.config[1][num,:])
                self.config[1][num,:] = deepcopy(self.config[5][:,num][::-1])

                temp2 = deepcopy(self.config[4][:,reflectedNum][::-1])
                self.config[4][:,reflectedNum] = temp1

                temp1 = deepcopy(self.config[3][reflectedNum,:])
                self.config[3][reflectedNum, :] = temp2

                self.config[5][:,num] = temp1
                ...
                temp1:np.ndarray = deepcopy(self.config[3][reflectedNum,:][::-1])
                self.config[3][reflectedNum,:] = deepcopy(self.config[5][:,num][::-1])

                temp2 = deepcopy(self.config[4][:,reflectedNum])
                self.config[4][:,reflectedNum] = temp1

                temp1 = deepcopy(self.config[1][num,:][::-1])
                self.config[1][num,:] = temp2
